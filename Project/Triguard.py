...
# my project banner 
banner = pyfiglet.figlet_format("Triguard")
...
try:
    try:
        ...
        if install_code != 0:
            ...
            quit()
        print('%s fuzzywuzzy has been installed, restart Web-Whisper.' % info)
        quit()
except ImportError:
    print('%s Web-Whisper isn\'t compatible with Python2.\n Use Python > 3.4 to run Web-Whisper.' % bad)
    quit()
...
# Run DotDotPwn after XSS tool completes
dotdotpwn_path = sys.path[0] + '/dotdotpwn-master/dotdotpwn.pl'
try:
    subprocess.run(['perl', dotdotpwn_path], check=True)
except subprocess.CalledProcessError as e:
    print(f'{bad} Failed to run DotDotPwn: {e}{end}')
